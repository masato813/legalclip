...
function parseParagraph(node: LawFullTextNode): ParsedParagraph {
  ...
  const items: ParsedItem[] = findChildrenByTag(node, "Item").map(itemNode => {
    const titleNode = findChildByTag(itemNode, "ItemTitle");
    const itemSentenceNode = findChildByTag(itemNode, "ItemSentence");
    const itemSentences: string[] = [];
    if (itemSentenceNode) {
      const sNodes = findChildrenByTag(itemSentenceNode, "Sentence");
      if (sNodes.length > 0) {
        sNodes.forEach(s => itemSentences.push(extractText(s)));
      } else {
        itemSentences.push(extractText(itemSentenceNode));
      }
    }
    return {
  ...
  return { num, paragraphNum, sentences, items };
}
